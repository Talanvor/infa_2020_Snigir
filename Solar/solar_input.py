    ...
    **space_objects** — список объектов планет и звёзд
    ...
    with open(output_filename, 'w') as out_file:
        for obj in space_objects:
            #fixed
            line = obj.type + " " + str(obj.R) + " " + obj.color + " " + str(obj.m) + " " + str(obj.x) + " " + str(obj.y) + " " + str(obj.Vx) + " " + str(obj.Vy) + "\n"
            out_file.write(line)

# FIXME: хорошо бы ещё сделать функцию, сохранающую статистику в заданный файл...
def writing_space_objects_data_as_stats(output_filename, space_objects):
    with open(output_filename, 'a') as stats:
        for obj in space_objects:
            line = obj.type + " " + str(obj.R) + " " + obj.color + " " + str(obj.m) + " " + str(obj.x) + " " + str(obj.y) + " " + str(obj.Vx) + " " + str(obj.Vy) + "\n"
            stats.write(line)
        stats.write('\n')
		
def file_clean(output_filename):
	filecleaning = open(output_filename, 'w')
	filecleaning.close()
